...
const AvailabilityManager = () => {
    ...
    const [queryIds, setQueryIds] = useState([LEGACY_ID]); // Oletuksena hakee edes vanhalla
    ...
    const [settings, setSettings] = useState({
        target_office_percent: 50, thursday_office_rate: 100, monday_always_remote: true,
        friday_max_presence_per_month: 1, primary_office_name: 'Malminkatu', thursday_office_name: 'Viipurinkatu'
    });

    const webCalUrl = expertId ? `webcal://espan-api.netlify.app/functions/webcal?expert_id=${expertId}` : '';

    const { 
        pendingReceipts, approvedReceipts, loading: journeyLoading, 
        approveReceipt, rejectReceipt, refreshReceipts, fetchApprovedReceipts, addVirtualReceipt
    } = useJourneyManager();
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            // Haetaan kirjautunut asiantuntija
            ...
            // Haetaan data molemmilla ID:illä (.in)
            const [rulesRes, excRes, locRes, setRes, ledgerRes, holidayRes, longDistRes] = await Promise.all([
                supabase.schema('espan').from('expert_availability_rules').select('*').in('expert_id', qIds).order('day_of_week').order('start_time'),
                supabase.schema('espan').from('availability').select('*').in('expert_id', qIds).gte('start_time', `${queryStart} 00:00:00`).lte('start_time', `${queryEnd} 23:59:59`).order('start_time'),
                supabase.schema('espan').from('expert_daily_locations').select('*').in('expert_id', qIds).gte('date', queryStart).lte('date', queryEnd),
                supabase.schema('espan').from('expert_location_settings').select('*').in('expert_id', qIds),
                supabase.schema('espan').from('expert_remote_bank_ledger').select('*').in('expert_id', qIds).gt('expiration_date', todayStr),
            ...
            // Asetukset: Suositaan uutta ID:tä, jos molemmilla on
            if (setRes.data && setRes.data.length > 0) {
                const activeSettings = setRes.data.find(s => s.expert_id === currentExpertId) || setRes.data[0];
            ...
            if (ledgerRes.data) {
                ...
                const usedLinkedIds = ledgerRes.data.filter(row => row.transaction_type === -1 && row.linked_earned_id).map(row => row.linked_earned_id);
                const availableDays = ledgerRes.data.filter(row => row.transaction_type === 1 && !usedLinkedIds.includes(row.id)).sort((a, b) => new Date(a.earned_date || a.created_at) - new Date(b.earned_date || b.created_at));
    ...
    // Tallennusfunktiot alikomponenteille (käytetään nyt expertId-tilaa)
    ...
    const handleOptimizeCalendar = async () => {
        if (!expertId) return;
        try {
            const response = await fetch('/.netlify/functions/optimize-calendar', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ expert_id: expertId }) });
    ...
    const handleLockCurrentWeek = async () => {
        try {
            ...
            const updates = weekDays.map(async (d) => {
                ...
                // Varmistetaan että lukitaan vain olemassa olevat rivit
    ...
    const handleResetSuggestions = async () => {
        ...
        try { 
            await supabase.schema('espan').from('expert_daily_locations').delete().in('expert_id', queryIds).eq('is_auto_generated', true); 
            fetchData(); 
        } catch (e) { console.error(e); }
    };

    const changeWeek = (offset) => setCurrentWeekStart(prev => addDays(prev, offset * 7));
    const jumpToCurrentWeek = () => setCurrentWeekStart(getMonday(new Date()));
                ...
                {activeView === 'locations' ? (
                    <div className="view-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                        ...
                        <LocationPlanner 
                            ...
                            longDistanceJourneys={longDistanceJourneys} 
                        />

                        {expertId && (
                            <ExpertCalendar 
                                expertId={expertId}
                                queryExpertIds={queryIds} // Välitetään molemmat poistoja varten
                                currentWeekStart={currentWeekStart}
                                dailyLocations={dailyLocations}
                                exceptions={exceptions}
                                rules={rules}
